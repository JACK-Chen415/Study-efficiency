...
本文档定义 MVP 主链路所需的 P0 REST API：简单登录、学习会话记录、可选手机运动特征、模型训练/预测和分析看板。完整认证、多角色管理和跨用户对比不属于 P0。

当前已实现的后端接口包括：`/api/users/simple-login`、`/api/sessions/start`、`/api/sessions/end`、`POST /api/sessions/{id}/abandon`、`/api/sessions/list`、`/api/sessions/{id}`、`PUT /api/sessions/{id}`、`DELETE /api/sessions/{id}`、`/api/motion/upload`、`/api/motion/{session_id}`、`POST /api/model/train`、`POST /api/model/predict`、`GET /api/model/metrics`、`GET /api/model/feature-importance`、`GET /api/analytics/overview`、`GET /api/analytics/trend` 与 `GET /api/analytics/factor-analysis`。
...
| 错误格式 | 当前使用 FastAPI 默认错误响应 `{"detail": "可读说明"}`；统一 `code/message/details` 可在后续 API 规范化时引入 |
...
`data_source` 约定：`real` 表示真实采集数据，`mock` / `demo` / `test` 表示流程验证数据。任何指标、图表或答辩口径中都必须区分，不得混称。
...
训练数据由 `ml/export_training_data.py` 从已完成、未放弃且带 `efficiency_score` 的学习记录导出，并 left join `motion_features`；缺失运动特征导出为 `0`，同时使用 `motion_available=0` 标识。后端模型接口已经上线，真实样本不足时会返回明确限制，不伪装成可靠结论。
...
限制说明：响应示例仅展示结构，不代表实验结果。真实训练完成后必须记录到 `EXPERIMENT_LOG.md`。样本少于 `30` 条真实有效 completed 记录时返回 `422` 与明确限制说明，不将演示结果表述为可靠结论。
...
返回最新模型的样本数、分类指标和混淆矩阵。若尚未训练模型，返回 `409`。

### `GET /api/model/feature-importance`

返回最新模型按重要性降序排列的特征列表。若尚未训练模型，返回 `409`。
...
统计接口只使用已完成、未放弃且带效率评分的会话；运动特征缺失记录仍纳入不依赖运动数据的统计。用户不存在返回 `404`，`user_id <= 0` 返回 `422`。
...
```json
{
  ...
  "avg_efficiency_score": 3.75,
  "high_efficiency_ratio": 0.4167,
  "label_distribution": {"low": 2, "medium": 5, "high": 5},
  "motion_available_count": 8,
  "latest_session_at": "2026-06-18T20:00:00",
  "latest_prediction": {
    "id": 901,
    "session_id": 101,
    "predicted_label": "high",
    "confidence": 0.78,
    "model_version": "rf_20260618_001",
    "suggestion": "保持明确学习目标，并继续减少手机干扰。",
    "created_at": "2026-06-18T21:21:00"
  },
...
```json
{
  ...
  "time_periods": [
    {
      ...
      "avg_duration_minutes": 60.0,
      "avg_efficiency_score": 4.0,
      "high_efficiency_ratio": 0.5
    }
  ],
  "motion_efficiency_points": [
    {
      "session_id": 101,
  ...
  "rule_suggestions": [
    {
      "code": "phone_distraction",
      "title": "强手机干扰建议",
      "message": "手机干扰达到 4 或 5 时，建议开启勿扰模式、移出视线范围，并用固定时间点集中查看消息。",
      "trigger_count": 2,
      "active": true
    }
  ],
  "model_snapshot": {
    ...
    "warnings": ["仅流程验证，不代表真实学习效率规律。"],
    "feature_importance": [
      {"rank": 1, "feature_name": "phone_distraction", "importance_score": 0.21}
    ]
  }
}
```

规则建议当前覆盖高疲劳、强手机干扰、目标清晰度低、噪声高和深夜学习五类。`model_snapshot.data_source` 为 `mock` / `demo` / `test` 时，只能用于系统演示，不得表述为真实学习规律。
