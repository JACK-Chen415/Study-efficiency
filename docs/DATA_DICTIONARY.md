...
| `motion_available` | 是运动特征是否采集到的缺失机制标记，不是学习行为本身 |
...
P0 使用用户自选昵称进入系统，不实现密码、token 或角色权限。同一昵称对应同一个用户；年级和专业是首次创建该昵称用户时的可选资料，不作为区分用户的键。昵称用于去标识化区分参与者，不应解释为绝对匿名身份。
...
用户主动放弃一次未结束学习，或因异常中断后被明确放弃时，只设置 `abandoned_at` 与 `abandon_reason`，不填写结束表单和效率评分。已放弃记录保留在数据库中用于追踪流程质量、异常中断或误操作，但不会阻塞同一用户重新开始学习，不进入默认历史列表，也不进入训练数据导出；`abandoned` 不能解释为低效率样本。
...
缺失处理原则：采集失败时可不存在该行；Milestone 4A 的训练数据导出阶段采用填 `0`，并在 CSV 中增加 `motion_available` 标识是否存在运动特征。`motion_available` 是缺失机制标记，不代表学习行为本身；运动字段导出填 `0` 只是为了形成稳定的建模矩阵，不代表真实场景中“完全没有运动”。正式训练和实验记录中仍必须写明该处理方式及限制。
...
| `motion_available` | `TINYINT UNSIGNED` | 是 | `0` / `1` | 删除时是否存在运动特征；缺失机制标记，不代表学习行为本身 |
...
1. `study_sessions` 中已完成、`end_time` 不为空、关键字段合法且 `efficiency_score` 不为空的记录。
...
以下字段可先作为导出、质检或人工清洗阶段的说明字段，不强制修改当前数据库表：

| 字段 | 建议取值 | 说明 |
| --- | --- | --- |
| `quality_flag` | `valid` / `too_short` / `duplicate_candidate` / `careless` / `test_data` / `outlier_duration` | 训练前质量标记；`duration_minutes < 5` 的 completed 记录默认标记为 `too_short`。 |
| `data_source` | `real` / `test` / `mock` / `demo` | 区分真实采集、功能测试、模拟或演示数据，报告中不得混称。 |
| `exclude_reason` | `abandoned` / `deleted` / `missing_label` / `too_short` / `invalid_field` / `test_or_mock` | 记录未进入正式训练集的原因，便于实验日志追溯。 |
